...
def EvaluatePerformance(
    ...
    noiseLevels = {
        ...
        "Simple_3.xes": 0.0,
        "Simple_4.xes": 0.0,
        "Intermediate_1.xes": 0.1,
        "Intermediate_3.xes": 0.1,
        "Intermediate_2.xes": 0.2,
        "Intermediate_4.xes": 0.2,
        "Difficult_1.xes": 0.2,
        "Difficult_3.xes": 0.2,
        "Difficult_2.xes": 0.35,
        "Difficult_4.xes": 0.35,
    }

    driftSpacing = {
        "Intermediate_1.xes": 667,
        "Intermediate_3.xes": 667,
        "Intermediate_2.xes": 400,
        "Intermediate_4.xes": 400,
        "Difficult_1.xes": 400,
        "Difficult_3.xes": 400,
        "Difficult_2.xes": 200,
        "Difficult_4.xes": 200,
    ...
    def AggregateMetric(results, metricKey, mapping):

        groupedValues = {}

        for logName, value in mapping.items():

            if logName not in results:
                continue

            metric = results[logName][metricKey]

            if metric is None:
                continue

            groupedValues.setdefault(value, []).append(metric)

        xValues = sorted(groupedValues.keys())

        yValues = [
            np.mean(groupedValues[value])
    ...
    def ComputeGroupAverage(results, groupPrefix):

        matchingLogs = [
            logName
            for logName in results.keys()
            if logName.startswith(groupPrefix)
        ]
        ...
        f1Scores = [
            results[logName]["F1-score"]
            for logName in matchingLogs
        ]

        precisionScores = [
            results[logName]["Precision"]
            for logName in matchingLogs
        ]

        recallScores = [
            results[logName]["Recall"]
            for logName in matchingLogs
        ]

        latencies = [
            results[logName]["Avg Latency"]
            for logName in matchingLogs
            if results[logName]["Avg Latency"] is not None
        ]

        return {
            "Average F1-score": np.mean(f1Scores),
            "Average Precision": np.mean(precisionScores),
            "Average Recall": np.mean(recallScores),
    ...
    for methodIndex, detectedDriftCoords in enumerate(methodResultsList):
        ...
        results["SIMPLE_AVERAGE"] = ComputeGroupAverage(results, "Simple")
        results["INTERMEDIATE_AVERAGE"] = ComputeGroupAverage(results, "Intermediate")
        results["DIFFICULT_AVERAGE"] = ComputeGroupAverage(results, "Difficult")
        ...
        for category in [
            ...
            print(f"\n{category}")
            print(f"Average F1-score  : {categoryResults['Average F1-score']:.4f}")
            print(f"Average Precision : {categoryResults['Average Precision']:.4f}")
            print(f"Average Recall    : {categoryResults['Average Recall']:.4f}")

            latency = categoryResults["Average Latency"]
    ...
    if plot:
        ...
        for methodName in methodNames:
            ...
            results = allResults[methodName]
            color = methodColors[methodName]

            # Noise versus F1-score
            noiseXAccuracy, noiseYAccuracy = AggregateMetric(
                results,
                "F1-score",
                noiseLevels
            )
            ...
            # Drift spacing versus F1-score
            spacingXAccuracy, spacingYAccuracy = AggregateMetric(
                results,
                "F1-score",
                driftSpacing
            )
            ...
            noiseXLatency, noiseYLatency = AggregateMetric(
                results,
                "Avg Latency",
                noiseLevels
            )
            ...
            # Drift spacing versus latency
            spacingXLatency, spacingYLatency = AggregateMetric(
                results,
                "Avg Latency",
                driftSpacing
            )
        ...
        axes[0, 0].set_title("Noise vs F1-score")
        axes[0, 0].set_xlabel("Noise Level")
        axes[0, 0].set_ylabel("F1-score")
        axes[0, 0].legend()

        axes[0, 1].set_title("Drift Spacing vs F1-score")
        axes[0, 1].set_xlabel("Drift Spacing")
        axes[0, 1].set_ylabel("F1-score")
        axes[0, 1].legend()

        axes[1, 0].set_title("Noise vs Latency")
        axes[1, 0].set_xlabel("Noise Level")
        axes[1, 0].set_ylabel("Latency")
        axes[1, 0].legend()

        axes[1, 1].set_title("Drift Spacing vs Latency")
        axes[1, 1].set_xlabel("Drift Spacing")
        ...
        plt.savefig(
            os.path.join(saveFolder, "combined_line_charts.png"),
            dpi=300,
            bbox_inches='tight'
        )
        ...
        barColors = [methodColors[methodName] for methodName in methodNames]


        plt.figure(figsize=(10, 6))

        plt.bar(methodNames, overallF1Scores, color=barColors, width=0.5)
        plt.ylim(0, 0.75)
        ...
        plt.bar(methodNames, overallPrecisions, color=barColors, width=0.5)
        plt.ylim(0, 0.75)
        ...
        plt.bar(methodNames, overallRecalls, color=barColors, width=0.5)
        plt.ylim(0, 0.75)
        ...
        plt.bar(methodNames, overallLatencies, color=barColors, width=0.5)
...
#BPIC2015Merged.xes with appropriate configuration [600, 1008, 2190, 2811, 4388]
